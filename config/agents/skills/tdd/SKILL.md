...
description: |
  ...
  - build features or fix bugs using TDD
...
They mock internal collaborators, test private methods, or verify through external means
(like querying a database directly instead of using the interface).
...
and [mocking.md](./references/mocking.md) for mocking guidelines.
If you're dealing with Golang, use `golang-testing` skill.
...
When exploring the codebase, use the project's domain glossary so that test names
and interface vocabulary match the project's language,
