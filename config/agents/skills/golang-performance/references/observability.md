...
Confirm metric names and semantics against the Go runtime,
collector, and monitoring versions used by the project.
Tune scrape intervals and profiling duration to keep observability overhead
...
Test PromQL expressions against a non-production or read-only Prometheus target.
Inspect returned labels and time ranges before deploying alert rules.

### Grafana Dashboards

Start with a maintained Go runtime dashboard compatible with the deployed
collector and metric names, then adapt it to the service's baseline.
