...
getSecret() {
  sops -d "$MONGO_SECRETS_PATH/$2/passwd$3" | grep "$1" | awk -F " " '{print $2}' | sed -e 's/"//g'
}

getTenantInfo() {
  mongoexport --host="127.0.0.1:8102" --db=lerta --username=admin --password="$1" --authenticationDatabase "admin" --collection=tenant -q='{}' --fields='_id,name,shortName' --jsonArray --pretty --quiet | jq
}
